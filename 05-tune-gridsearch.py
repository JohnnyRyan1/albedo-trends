...
# Generate tapered or flat layer sizes
def generate_layer_sizes(num_layers, base_units, taper=False):
...
# Grid search
dense_layer_options = [2, 3, 4]
residual_options = [0, 1]
base_units_options = [64, 128]
tapering_options = [False, True]
activation_options = ['gelu', 'leaky_relu']
learning_rate_options = [0.01, 0.001]
dropout_rate_options = [0.1, 0.2, 0.3]

results = []

config_space = list(product(
    dense_layer_options,
    residual_options,
    base_units_options,
    tapering_options,
...
for (
    num_layers, res_blocks, base_units, taper,
    activation, lr, dropout
) in tqdm(config_space, desc="Benchmarking"):
    ...
    mse = benchmark_model(
        X_train, y_train, X_test, y_test,
        layer_sizes=layer_sizes,
        num_residual_blocks=res_blocks,
        activation=activation,
