...
slope_df['avg_abs_slope'] = slope_df.abs().mean(axis=1)
slope_df_sorted = slope_df.sort_values(by='avg_abs_slope', ascending=False)
slope_df_sorted = slope_df_sorted.drop(columns='avg_abs_slope')
...
cbar = fig.colorbar(c, ax=ax_main, pad=-0.1)
cbar.set_label('Albedo trend (per decade)', fontsize=12)
...
ax_line.plot(x, secondary_line, color='black', alpha=0.5, linewidth=2)
ax_line.set_ylabel('Mean trend', fontsize=12)
...
plt.savefig(path + 'figures/4-heatmaps/' + 'figX-heatmap-linear.png', dpi=300)
...
new_df.to_csv(path + 'data/trends.csv')
